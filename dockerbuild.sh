...
# 构建 full 版本的镜像
docker build --build-arg IMAGE_TYPE=full -t chunyeah/gpt-sovits:latest .
# 为同一个镜像添加带日期的标签
docker tag chunyeah/gpt-sovits:latest chunyeah/gpt-sovits:dev-$DATE
# 为同一个镜像添加带当前代码库Commit哈希值的标签
docker tag chunyeah/gpt-sovits:latest chunyeah/gpt-sovits:dev-$COMMIT_HASH
